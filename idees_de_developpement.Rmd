...
# Faire une fonction Décision Possible Rapide
Utiliser le dictionnaire universel des (play, take) pour réduire les possibilités et lister toutes les (play, take) associées à un hand et un board -> pour chaque play possible, restreindre le dictionnaire à ce play, mais même comme ça la recherche peut être encore longue car plus de 1000 possibilités pour un 10 -> soit on vérifie pour tous les takes associés si les cartes sont sur le board (all take %in% board), soit on trouve un moyen de restreindre la recherche à partir de l'info des cartes qui sont sur le board. Par exemple on peut faire des sous-dictionnaires redondants selon chacune des cartes présentes (donc pour takeable_dict[[D6]] on met des sous-entrées pour [[B1]], [[C1]], … jusqu'à [[S6]]). On peut aussi, pour chaque play envisagé, filtrer du board toutes les cartes ayant une valeur supérieure, et aussi faire la condition que s'il existe une carte de la même valeur on est obligé de la choisir avant d'explorer le reste. -> ces 2 mesures simples devraient rendre l'énumération rapide dès que le play n'est pas trop grand. On peut aussi analyser les values du board, et faire des sous-dictionnaires par values, comme ça on filtre tous les sous dictionnaires n'ayant pas les bonnes values...
...
1. on a en main un D7 et un 8, il y a sur le board un 5, 2, 1 -> utilise-t-on le D7 pour gagner le sette bello ou le 8 pour scoper -> ça devrait dépendre de la probabilité qu'other ait un 1 pour scoper derrière, entre autres choses
